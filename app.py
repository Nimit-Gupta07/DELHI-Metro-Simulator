...
def name_checker(station):
    ...
    if station in all_station_list:
        return re_ask,[] # -------change------ also return empty suggestions list
    ...
    if len(possible_station)>0:
        ...
        suggestions=[possible_station[i][0] for i in range(min(4,len(possible_station)))] # showing not more than 4 stations
        return re_ask,suggestions
    else :
        return re_ask, []
...
#we have too many interchanges manually taking into account all of them is tedios hence we take canstant interchange time
    ...
    if hour>22:
        return "No service available"#we define a breaking condition no service after 11
...
st.set_page_config(page_title="Delhi Metro Simulator")
st.title("Delhi Metro Route and Schedule Simulator")
st.write("Welcome! Use the options below to check metro timings or plan your journey.")

module = st.radio("Select a module:", ["Metro Timing Module","Ride Journey Planner"])

def validate_station(station_raw):
    #Returns (station_casefolded,is_valid,suggestions_list)
    station = station_raw.strip().casefold()
    if not station:
        return station,False,[]
    re_ask,suggestions=name_checker(station)
    if not re_ask:
        return station, True, []
    return station,False,suggestions

def validate_time(time_str, valid_time=False):
    #Returns True if time string HH:MM is valid within metro operating hours.
    try:
...
# ===================== MODULE 1: METRO TIMING =====================
if module == "Metro Timing Module":
    st.subheader("Metro Timing Module")
    line = st.selectbox("Select your metro line:", ["blue","pink","magenta"])
    station_raw = st.text_input("Enter your station name:")
    time_str = st.text_input("Enter time (HH:MM, 24-hour format, between 06:00 and 23:30):")
    if st.button("Get Metro Timings"):
        # validate station
        station, is_valid_station, suggestions = validate_station(station_raw)

        if not is_valid_station:
            if suggestions:
                st.warning("Station not found. Did you mean one of these?")
                for s in suggestions:
                    st.write(f" {s}")
            else:
                st.error("Station not found. Please check the spelling.")
        elif not validate_time(time_str, valid_time=True):
            st.error("Timing should be between 06:00 and 23:30 and in correct HH:MM format.")
        else:
            try:
                # resolve blue line to blue1 or blue2 based on which list the station is in
                resolved_line = line
                if line == "blue":
                    if station in stations_list[0]:
                        resolved_line = "blue1"
                    elif station in stations_list[1]:
                        resolved_line = "blue2"

                line_number = num_line_internal[resolved_line]

                # forward direction
                if station!=stations_list[line_number][-1]:
                    final_output, final_list= metro_timing_module(resolved_line, station, time_str, stations_list, time_list)
                    st.success(f"{final_output} towards {stations_list[line_number][-1]}")

                # reverse direction
                if station != reversed_station_list[line_number][-1]:
                    final_output, final_list =metro_timing_module(resolved_line, station, time_str, reversed_station_list, reversed_time_list)
                    st.success(f"{final_output} towards {reversed_station_list[line_number][-1]}")

            except Exception:
                st.error("The station doesn't lie on the selected line. Please check your inputs.")


# ===================== MODULE 2: JOURNEY PLANNER =====================
elif module=="Ride Journey Planner":
    st.subheader("Ride Journey Planner")
    source_raw= st.text_input("Enter source station:")
    dest_raw =st.text_input("Enter destination station:")

    time_str=st.text_input("Enter time (HH:MM, 24-hour format, between 06:00 and 23:00):")
    if st.button("Plan My Journey"):
        source, is_valid_src, suggestions_src= validate_station(source_raw)
        dest, is_valid_dst, suggestions_dst=validate_station(dest_raw)

        # show source suggestions if invalid
        if not is_valid_src:
            if suggestions_src:
                st.warning("Source station not found. Did you mean:")
                for s in suggestions_src:
                    st.write(f" {s}")
            else:
                st.error("Source station not found. Please check the spelling.")

        elif not is_valid_dst:
            if suggestions_dst:

                st.warning("Destination station not found. Did you mean:")
                for s in suggestions_dst:
                    st.write(f" {s}")
            else:
                st.error("Destination station not found. Please check the spelling.")

        elif not validate_time(time_str,valid_time=False):     
            st.error("Timing should be between 06:00 and 23:00 and in correct HH:MM format.")

        elif source == dest:
            st.warning("Source and destination stations need to be different.")
        else:
            try:
                result = journey_planner(source,dest,time_str)
                
                st.success(result)
            except Exception:
                st.error("Could not calculate journey. Please check your inputs or try a different time.")